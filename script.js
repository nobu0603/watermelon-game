...
function createRandomFallingObject(x, y) {
  ...
  const scale =
    (objectDef.size * 2) /
    Math.max(objectDef.originalWidth, objectDef.originalHeight);
...
function mergeBodies(pair) {
  ...
  if (bodyA.label === bodyB.label) {
    ...
    if (nextObjectDef) {
      ...
      // スケールを計算（オブジェクトのサイズに合わせる）
      const scale =
        (nextObjectDef.size * 2) /
        Math.max(nextObjectDef.originalWidth, nextObjectDef.originalHeight);
